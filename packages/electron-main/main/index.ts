...
app.on("ready", async () => {
  if (isDev) {
    ...
    const viteServer = await createServer({
      ...
      envDir: join(dirname(import.meta.filename), "../../.."),
      server: {
        port: 5173,
      },
    });
...
const rendererClientPath = join(__dirname, "../renderer");
...
ipcMain.handle("ipcTest", (_event, ...args) =>
  console.debug("ipc: renderer -> main", { ...args }),
);
...
(async () => {
  ...
  protocol.handle("http", async (req) => {
    ...
    try {
      ...
      const indexPath = join(rendererClientPath, "index.html");
